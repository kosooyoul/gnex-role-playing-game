...
연습8일 + 
 ...
 9일차: 3월 8일 : 서버/클라이언트 구현 연습(예뮬레이터에선 잘 작동함/서버는 VB로 구현-다른 언어로 작성하여 SQL연동 요망)
10일차: 3월 9일 : 서버/클라이언트 구현 연습(핸드폰에서 네트워크 접속시 로컬네트워크로 접속되지 않으므로 서버는 전용선이거나 VPN설치가 필요)
11일차: 3월10일 : 선택지 구현(간단 예/아니오 차후 확장 요망), 


**이벤트 함수 추가요망(상점, 선택지 등)
